...
if __name__ == '__main__':
    ...
    fig1 = plt.figure()
    fig2 = plt.figure()
    for c, N in enumerate([4, 5, 10, 100]): # トレーニングセットのデータ数
        train_set = generate_training_set(N)
        mean_fun, deviation_fun, mean, s = resolve(train_set, order)
        ws_samples = DataFrame(multivariate_normal(mean, s, 4))
